...
const statusColumns: Array<{ id: TaskStatus; label: string; bg: string; text: string; border: string }> = [
  { id: TaskStatus.TODO, label: 'To Do', bg: 'bg-[#F1F1EF]', text: 'text-slate-650', border: 'border-[#EDEDEB]' },
  { id: TaskStatus.IN_PROGRESS, label: 'In Progress', bg: 'bg-[#EBF5FE]', text: 'text-[#0369A1]', border: 'border-[#E0F2FE]' },
  { id: TaskStatus.REVIEW, label: 'In Review', bg: 'bg-[#FEF9C3]/50', text: 'text-[#A16207]', border: 'border-[#FEF9C3]' },
  { id: TaskStatus.DONE, label: 'Completed', bg: 'bg-[#ECFDF5]', text: 'text-[#047857]', border: 'border-[#D1FAE5]' },
]
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start overflow-x-auto pb-4">
          {statusColumns.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className="flex flex-col bg-[#F7F7F5]/50 border border-[#EDEDEB] rounded-xl p-3.5 min-h-[450px]"
              >
                {/* Column Header */}
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="p-1 rounded text-muted hover:text-slate-900 hover:bg-black/[0.04] transition-colors"
                ...
                <div className="flex-1 flex flex-col gap-2.5 min-h-[350px]">
                  <AnimatePresence initial={false}>
                    {colTasks.length === 0 ? (
                      <div className="flex-1 flex flex-col items-center justify-center p-6 border-2 border-dashed border-[#EDEDEB] rounded-lg text-slate-305 text-center select-none">
                        <ClipboardList className="w-6 h-6 text-slate-300 mb-1.5 opacity-50" />
                      ...
                      colTasks.map((task) => (
                        <motion.div
                          ...
                          className="group relative bg-white border border-[#EDEDEB] p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer"
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-1.5 mb-1.5">
                            ...
                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                              <button
                                onClick={() => handleOpenEdit(task)}
                                className="p-1 text-slate-400 hover:text-slate-900 rounded hover:bg-black/[0.04]"
                                title="Edit"
                              >
                              ...
                              <button
                                ...
                                className="p-1 text-slate-400 hover:text-danger rounded hover:bg-danger/10"
                                title="Delete"
                              >
                          ...
                          <h4 className="text-sm font-bold text-slate-800 line-clamp-2 leading-snug mb-1">{task.title}</h4>
                          {task.description && (
                            <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed mb-3">{task.description}</p>
                          )}

                          {/* Footer */}
                          <div className="flex items-center justify-between border-t border-[#EDEDEB]/50 pt-2 mt-2 gap-1.5">
                ...
                {colTasks.length > 0 && (
                  <button
                    onClick={() => handleOpenCreate(col.id)}
                    className="mt-3.5 w-full flex items-center justify-center gap-1 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-900 hover:bg-black/[0.03] border border-transparent hover:border-[#EDEDEB] rounded-lg transition-all duration-150"
                  >
        ...
        {activeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm">
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white border border-[#EDEDEB] rounded-xl shadow-modal max-w-md w-full overflow-hidden p-6 z-50 relative"
            >
              <button
                onClick={() => setActiveModal(null)}
                className="absolute top-4 right-4 p-1 rounded-lg text-muted hover:text-slate-955 hover:bg-black/[0.04] transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
